...
public class UserServiceImpl implements UserService {
	...
	public void add(TbUser user) {
		...
		String passowrd = DigestUtils.md5Hex(user.getPassword());
	...
	public PageResult findPage(TbUser user, int pageNum, int pageSize) {
		...
		public void createSmsCode(String phone) {
			String code1=(long) (Math.random()*1000000)+"";
			System.out.println("验证码："+code1);
			redis.boundHashOps("smscode").put(phone, code1);
			
			
			System.out.println("签名是："+sign_name);
			System.out.println("模板："+template_code);
			
			//发送
			jms.send(smsDestination, new MessageCreator() {
				
				@Override
				public Message createMessage(Session session) throws JMSException {
					MapMessage message = session.createMapMessage();
					message.setString("mobile", phone);    //手机
		...
		public boolean checksmscode(String phone, String code) {
		      ...
		      System.out.println("传递的"+code);
		      System.out.println("redisde:"+object);
		      if(object==null) {
		    	  return false;
		      }
